...
class PackageUpgrader {
  ...
  async batchUpgradeCompatiblePackages(packages) {
    ...
    try {
      ...
      const command = `npm install ${upgradeCommands.join(' ')} --no-audit --no-fund --prefer-offline`;
  ...
  async upgradePackageFast(packageName, targetVersion) {
    ...
    try {
      ...
      await this.execAsync(`npm install ${packageName}@${targetVersion} --no-audit --no-fund --prefer-offline`, { 
        silent: true, 
        timeout: 90000 
      });
      ...
      if (validation.success) {
        ...
        //reason = reason.split('\n')[0]; // keep only first line for readability        
  ...
  async upgradeSequentiallyOptimized() {
    ...
    const riskyPackages = this.upgradeSequence.filter(pkg => {
      const packageData = this.discoveredPackages.get(pkg.name);
      return packageData.riskLevel === 'medium-risk' || packageData.riskLevel === 'high-risk';
    });
