...
const DEPENDENCY_SEQUENCE = {
    ...
    'typescript': { order: 3, peers: [], ecosystem: 'build' },
    '@mui/material': { order: 4, peers: ['@emotion/react', '@emotion/styled'], ecosystem: 'mui' },
    '@emotion/react': { order: 5, peers: [], ecosystem: 'mui' },
    '@emotion/styled': { order: 6, peers: ['@emotion/react'], ecosystem: 'mui' },
    '@azure/msal-browser': { order: 7, peers: [], ecosystem: 'azure' },
    '@azure/msal-react': { order: 8, peers: ['@azure/msal-browser', 'react'], ecosystem: 'azure' },
    'eslint': { order: 9, peers: [], ecosystem: 'lint' },
    '@typescript-eslint/eslint-plugin': { order: 10, peers: ['typescript', 'eslint'], ecosystem: 'lint' },
    '@typescript-eslint/parser': { order: 11, peers: ['typescript', 'eslint'], ecosystem: 'lint' },
    '@types/react': { order: 12, peers: ['react'], ecosystem: 'types' },
    '@types/react-dom': { order: 13, peers: ['react-dom', '@types/react'], ecosystem: 'types' },
    '@types/jest': { order: 14, peers: [], ecosystem: 'types' },
    '@mui/icons-material': { order: 15, peers: ['@mui/material'], ecosystem: 'mui' },
    '@mui/lab': { order: 16, peers: ['@mui/material'], ecosystem: 'mui' },
    '@testing-library/react': { order: 17, peers: ['react'], ecosystem: 'test' },
    '@testing-library/jest-dom': { order: 18, peers: [], ecosystem: 'test' },
    '@fontsource/roboto': { order: 19, peers: [], ecosystem: 'fonts' },
    '@microsoft/applicationinsights-react-js': { order: 20, peers: [], ecosystem: 'monitoring' },
    '@mui/x-data-grid': { order: 21, peers: [], ecosystem: 'mui' },
    'axios': { order: 22, peers: [], ecosystem: 'utilities' },
    'cronstrue': { order: 23, peers: [], ecosystem: 'utilities' },
    'react-h5-audio-player': { order: 24, peers: [], ecosystem: 'react-components' },
    'react-js-cron': { order: 25, peers: [], ecosystem: 'react-components' },
    'react-router-dom': { order: 26, peers: ['react'], ecosystem: 'routing' },
    'web-vitals': { order: 27, peers: [], ecosystem: 'performance' },
    'cross-env': { order: 28, peers: [], ecosystem: 'build' },
    'eslint-plugin-unused-imports': { order: 29, peers: [], ecosystem: 'lint' },
    'husky': { order: 30, peers: [], ecosystem: 'git' },
    'postcss-preset-env': { order: 31, peers: [], ecosystem: 'build' },
    'prettier': { order: 32, peers: [], ecosystem: 'formatting' }
};
...
// Ecosystem upgrade groups
const ECOSYSTEMS = {
    react: ['react', 'react-dom', '@types/react', '@types/react-dom'],
    mui: ['@mui/material', '@mui/icons-material', '@mui/lab', '@emotion/react', '@emotion/styled', '@mui/x-data-grid'],
    azure: ['@azure/msal-browser', '@azure/msal-react'],
    lint: ['eslint', '@typescript-eslint/eslint-plugin', '@typescript-eslint/parser', 'eslint-plugin-unused-imports'],
    types: ['@types/react', '@types/react-dom', '@types/jest'],
    test: ['@testing-library/react', '@testing-library/jest-dom'],
    build: ['typescript', 'cross-env', 'postcss-preset-env'],
    utilities: ['axios', 'cronstrue', 'web-vitals'],
    routing: ['react-router-dom'],
    fonts: ['@fontsource/roboto'],
    monitoring: ['@microsoft/applicationinsights-react-js'],
    formatting: ['prettier'],
    git: ['husky'],
    'react-components': ['react-h5-audio-player', 'react-js-cron']
};
...
class PackageUpgrader {
    ...
    buildUpgradeSequence() {
        ...
        // Build final sequence: ecosystems in order, then packages within ecosystems
        this.upgradeSequence = [];
        
        // Add known ecosystems in logical order
        const ecosystemOrder = ['react', 'types', 'mui', 'azure', 'lint', 'test', 'build', 'utilities', 
                               'routing', 'fonts', 'monitoring', 'formatting', 'git', 'react-components', 'other'];
    ...
    async validateUpgrade(ecosystem = null) {
        // Only run validation for high-risk packages or after ecosystem upgrades
    ...
    async upgradeEcosystemPackages(ecosystem, packages) {
        ...
        try {
            ...
            // Try dry run first
            try {
                await this.exec(`npm install ${installCommands.join(' ')} --dry-run`, { silent: true });
            } catch (dryRunError) {
            ...
            await this.exec(`npm install ${installCommands.join(' ')}`, { silent: true });
            
            // Validate the upgrade (only for high-risk ecosystems)
            const ecosystemRisk = packages.some(pkg => {
                const info = this.discoveredPackages.get(pkg);
                return info.riskLevel === 'high-risk';
            });
            ...
            this.error(`${ecosystem} ecosystem upgrade failed: ${error.message}`);
            await this.restoreBackup();
    ...
    async upgradePackage(packageName, targetVersion) {
        ...
        try {
            ...
            // Only validate for high-risk packages
    ...
    async upgradeBatch(packages) {
        if (packages.length === 0) {
            return true;
        }

        this.log(`\n🚀 Attempting batch install for ${packages.length} packages...`);
        ...
        try {
            ...
            try {
                ...
                if (this.isPeerDependencyConflict(dryRunError.message)) {
                    const conflictReason = this.extractPeerDependencyInfo(dryRunError.message);
                    this.log(`⚠️  Batch install has peer dependency conflict: ${conflictReason}`);
                    this.log(`🔄 Will attempt upgrade anyway and handle conflicts...`);
                }
            }
            ...
            const hasHighRisk = packages.some(pkg => {
                const info = this.discoveredPackages.get(pkg);
                return info.riskLevel === 'high-risk';
            });
            ...
            if (validation.success) {
                ...
                this.error(`Batch install failed validation: ${validation.failedStep}`);
                await this.restoreBackup();
            ...
            this.error(`Batch install failed: ${error.message}`);
            await this.restoreBackup();
    ...
    async upgradeSequentially() {
        ...
        for (let i = 0; i < this.upgradeSequence.length; i++) {
            ...
            // Skip if already processed (e.g., as a peer dependency or ecosystem upgrade)
            if (this.results.successful.some(p => p.name === packageInfo.name)) {
                this.log(`⏭️  Skipping ${packageInfo.name} (already upgraded)`);
                continue;
            }

            if (this.results.failed.some(p => p.name === packageInfo.name)) {
                this.log(`⏭️  Skipping ${packageInfo.name} (already failed)`);
                continue;
            }
        ...
        if (!finalValidation.success) {
            ...
            // Don't mark as failed, but log the issue
        }
        ...
        this.log(`  Total Time: ${Math.round((Date.now() - this.startTime) / 60000)} minutes`);
    }
    ...
    generateEmailReport() {
      ...
      <ul>
        <li>Plan coordinated ecosystem upgrades for conflicted packages</li>
        <li>Consider grouping React, Material UI, or MSAL packages together</li>
        <li>Review peer dependency requirements for major version planning</li>
        <li>Check for breaking changes in major version upgrades</li>
      </ul>`;
        }
...
async function main() {
    ...
    try {
        ...
        if (category === 'all') {
            ...
            // For safe packages, try batch install first
            if (category === 'all-safe') {
                const safePackageNames = filteredSequence.map(pkg => pkg.name);
                const batchSuccess = await upgrader.upgradeBatch(safePackageNames);
                
                if (!batchSuccess) {
                    upgrader.log('Falling back to sequential upgrade for failed packages...');
