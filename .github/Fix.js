...
// Default coordinated group definitions
...
class PackageUpgrader {
    ...
    detectNewPackages() {
        try {
            ...
            let outdated = {};
            try {
                const out = this.exec('npm outdated --json --long', { silent: true });
                outdated = out ? JSON.parse(out) : {};
            } catch (err) {
                try {
                    const out2 = err.stdout ? err.stdout.toString() : '';
                    outdated = out2 ? JSON.parse(out2) : {};
                } catch (parseErr) {
                    this.log('No outdated packages found or unable to parse npm outdated output');
                    outdated = {};
                }
            }
            ...
            for (const [packageName, currentVersionSpec] of Object.entries(currentPackages)) {
                ...
                if (outdated[packageName] && outdated[packageName].latest) {
                ...
                // Handle TypeScript with CRA constraint
                if (packageName === 'typescript') {
                    ...
                    if (hasCRA) {
                        try {
                            const latest4 = this.exec('npm view typescript@^4 version --json', { silent: true }).trim();
                            const versions = JSON.parse(latest4);
                            if (Array.isArray(versions) && versions.length > 0) {
                                const ts4Latest = versions[versions.length - 1];
                                if (ts4Latest && ts4Latest.startsWith('4.')) {
                                    this.log(`🔒 Locking typescript to latest 4.x (${ts4Latest}) because react-scripts is present`);
                                    latestVersion = ts4Latest;
                                }
                            }
                        } catch (err) {
                            // If we can't get TypeScript 4.x, continue with the latest
                        }
                    }
                }

                if (!this.compareVersions(cleanVersion, latestVersion)) continue;
    ...
    compareVersions(current, latest) {
        // Use a proper semver comparison if available, otherwise use string comparison
        try {
            const semver = require('semver');
            return semver.lt(current, latest);
        } catch (e) {
            // Fallback to string comparison if semver is not available
    ...
    getLatestVersion(packageName) {
        try {
            const result = this.exec(`npm view ${packageName} version --json`, { silent: true });
            const output = result.trim();
            
            // Handle both string and array responses
            if (output.startsWith('[')) {
                const versions = JSON.parse(output);
                return Array.isArray(versions) ? versions[versions.length - 1] : null;
    ...
    async validateUpgrade(packageName, riskLevel) {
        try {
            ...
            const validationSteps = [
                ...
                { name: 'Running tests', command: 'npm test -- --bail --detectOpenHandles --runInBand', timeout: 300000 }
            ];

            for (const step of validationSteps) {
                this.log(`${step.name}...`);
                try {
                    this.exec(step.command, { silent: true, timeout: step.timeout });
        ...
        for (const [groupName, groupDef] of Object.entries(groups)) {
            ...
            try {
                if (validationMode === 'none') {
                    ...
                    this.exec('npm test -- --bail --detectOpenHandles --runInBand', { silent: true, timeout: 300000 });
                }
                for (const p of pkgs) {
                    const info = this.discoveredPackages.get(p) || {};
                    this.results.successful.push({
