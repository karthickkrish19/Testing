...
class PackageUpgrader {
    ...
    if (category === 'all-safe') {
        ...
        if (batch.length > 0) {
            this.log(`
🚀 Attempting batch install for ${batch.length} safe/low-risk packages...`);
            try {
                execSync(`npm install ${batch.join(' ')} --no-audit --no-fund`, { stdio: 'inherit' });
                ...
                for (const pkg of batch) {
                    try {
                        ...
                        this.log(`✅ Installed ${pkg} individually`);
                        const [name, version] = pkg.split('@');
                        this.results.successful.push({
                            name,
                            oldVersion: this.getCurrentVersion(name),
                        ...
                        this.log(`❌ Failed to install ${pkg}: ${err.message}`);
                        const [name, version] = pkg.split('@');
                        this.results.failed.push({
                            name,
                            oldVersion: this.getCurrentVersion(name),
